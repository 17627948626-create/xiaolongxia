...
- 2026-05-15 下午:排查 Hermes 小龙虾 Feishu DM 双发/露出 `NO_REPLY` 问题，根因是沿用了 OpenClaw 旧口径“普通 assistant final 不会投递到飞书，必须 `message.send`”。Hermes gateway 会把普通 final 直接发回当前 DM，因此当前活规则改为：老板直接 DM 时普通 final 回复即可，禁止为同一段文字调用 `message.send`，也禁止输出 `NO_REPLY`；`message` 工具只留给 cron/background 主动通知和二维码/图片等普通文本做不了的场景。
...
- 2026-05-15 上午:【已被 2026-05-15 下午 Hermes 口径废弃】当时误判 Feishu direct 场景必须显式调用 `message.send`，并要求 final 输出 `NO_REPLY`；迁移到 Hermes 后已确认这是 OpenClaw 旧语义，不再作为活规则。
...
- 2026-04-28 傍晚:修复本虾发文进度重复投递规则。根因复盘为主 agent 没有严格把子 agent completion event 当内部信号处理，导致 completion event 与自然回复/主动通知叠加。当前仍保留“completion event 默认只更新 durable state、不直接对外播报”的规则；其中旧 `NO_REPLY` 表达已在 2026-05-15 Hermes 迁移后废弃。
...
- Feishu DM 通知与回复原则（Hermes 口径）：老板直接在 Feishu DM 发来的消息，普通 final 就是可见回复；不要为同一段文字再调 `message.send`，也不要输出 `NO_REPLY`。`message` 工具只用于 cron/background 主动通知、扫码/图片等普通文本回复做不了的场景。
